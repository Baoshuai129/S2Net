...
def parse_args():
    ...
    # parser.add_argument("--model_name", type=str, default='HFETNet_HCI_2x2-7x7')
    parser.add_argument("--testset_dir", type=str, default='../Data/TestData_RE_HCI_2x2-7x7/')


    # parser.add_argument("--model_name", type=str, default='extro2_DistgASR_Lytro_2x2-8x8')
    # parser.add_argument("--testset_dir", type=str, default='../Data/TestData_RE_Lytro_2x2-8x8_extro2/')
    #
    # parser.add_argument("--model_name", type=str, default='train2222-DistgASR_Lytro_2x2-7x7')
    # parser.add_argument("--testset_dir", type=str, default='../Data/TestData_RE_Lytro_2x2-7x7/')
    ...
    parser.add_argument('--save_path', type=str, default='./Results/weight/')
...
def test(cfg):
    ...
    model = torch.load('./log/' + cfg.model_name + '.pth.tar', map_location={'cuda:3': cfg.device})
            ...
            for u in range(numU):
                for v in range(numV):
                    ...
                    with torch.no_grad():
                        ...
                        # print("time：%.5f秒" % (end-start))
                        cost_time += end - start


                        subLFout[u, v, :, :] = out.squeeze()

            # print("推理时间：%.5f秒" % cost_time)
