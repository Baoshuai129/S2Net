...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, index):       #复写 __getitem__ ，同时返回数据和对应的标签（Tensor类型）
    ...
    def __len__(self):      #复写 __len__ 方法来获取数据的个数
...
def augmentation(data, label):
    # set_random_seed()
    if random.random() < 0.6:  # flip along W-V direction
        data = data[:, ::-1]
        label = label[:, ::-1]
        # print("a1")
    if random.random() < 0.6:  # flip along W-V direction
        data = data[::-1, :]
        label = label[::-1, :]
    if random.random() < 0.6: # transpose between U-V and H-W
...
def cal_psnr(img1, img2):
    ...
    out = metrics.peak_signal_noise_ratio(img1_np, img2_np)
...
def cal_ssim(img1, img2):
    ...
    datarange1 = dmax - dmin
    # print(datarange1)
    out = metrics.structural_similarity(img1_np, img2_np, data_range=datarange1, gaussian_weights=True, sigma=1.5, use_sample_covariance=False)
...
def cal_metrics_RE(img1, img2, angRes_in, angRes_out):
    ...
    for u in range(1, angRes_out -1 , (angRes_out - 3) // (angRes_in - 1)):
        for v in range(1, angRes_out - 1, (angRes_out - 3) // (angRes_in - 1)):
            PSNR[u, v] = 0
            SSIM[u, v] = 0
