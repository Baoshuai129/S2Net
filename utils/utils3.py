...
def augmentation(data, label):
    if random.random() < 0.7:  # flip along W-V direction
        data = data[:, ::-1]
        label = label[:, ::-1]
    if random.random() < 0.7:  # flip along W-V direction
        data = data[::-1, :]
        label = label[::-1, :]
    if random.random() < 0.7: # transpose between U-V and H-W
...
def cal_ssim(img1, img2):
    img1_np = img1.data.cpu().numpy()
    img2_np = img2.data.cpu().numpy()
    dmin, dmax = dtype_range[img2_np.dtype.type]
    datarange1 = dmax - dmin
    # print(datarange1)
    out = metrics.structural_similarity(img1_np, img2_np, data_range=datarange1, gaussian_weights=True, sigma=1.5,
                                        use_sample_covariance=False)
    # out = metrics.structural_similarity(img1_np, img2_np, gaussian_weights=True, sigma=1.5, use_sample_covariance=False)

    return out

def cal_metrics_RE(img1, img2, angRes_in, angRes_out, file_path, file_name):
    if len(img1.size())==2:
        [H, W] = img1.size()
    ...
    for u in range(U):
        ...
        # 2x2-8x8 Extra_1:
        # for u in range (1, angRes_out, 5):
        #     for v in range (1, angRes_out, 5):
        #         PSNR[u, v] = 0
        #         SSIM[u, v] = 0

        # 2x2-8x8 Extra_2:
        # for u in range (2, angRes_out, 3):
        #     for v in range (2, angRes_out, 3):
        #         PSNR[u, v] = 0
        #         SSIM[u, v] = 0
        ################################
    ...
    for u in range(U):
        for v in range(V):
            txtfile = open(file_path + '/' + str(file_name) + '_metrics.txt', 'a')
            txtfile.write('u: ----%d, v: ----%d, PSNR---%f, SSIM---%f\n' % (u + 1, v + 1, PSNR[u, v], SSIM[u, v]))
            txtfile.write('------------------------------------------\n')
            txtfile.close()
            pass
        pass
